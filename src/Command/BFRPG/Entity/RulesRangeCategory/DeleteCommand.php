 ...
 */
#[AsCommand(
    name: 'app:bfrpg:entity:rules-range-category:delete',
    description: 'Delete a rules range category'
)]
final class DeleteCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->title('BFRPG: Delete Rules Range Category');
        ...
        try {
            ...
            if ($input->isInteractive()) {
                ...
                if (!$io->confirm('Delete rules range category?')) {
                    return Command::SUCCESS;
                }
            }

            $id = $rangeCategory->getId();

            $this->entityManager->remove($rangeCategory);
            $this->entityManager->flush();

            $io->success(sprintf('Rules range category with id %d has been deleted.', $id));
        } catch (Throwable $e) {
            $this->logThrowable($e);
